#!/usr/bin/env bash
# Midnight Obsidian - Automatic Setup Script for Arch Linux / Codespaces
...
# Ensure yay can run without interactive prompts
...
for dir in hypr kitty waybar rofi swaync neofetch; do
    if [ -d "$dir" ]; then
        echo "Copying $dir to $CONFIG_DIR..."
...
if [ -d "Wallpaper" ]; then
    ...
    cp -r Wallpaper/* "$CONFIG_DIR/hypr/Wallpaper/"
fi
...
if [ -d "sddm/windows_7" ]; then
    echo "Installing Windows 7 SDDM theme..."
    sudo cp -r sddm/windows_7 /usr/share/sddm/themes/
fi
